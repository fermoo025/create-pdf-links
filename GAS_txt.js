...
function matchPdfUsingINumericPrice() {
  ...
  while (files.hasNext()) {
    ...
    if (manMatch) {
      ...
      const yenMatch = text.match(/価\s*格[:\s]*([\d,]+)\s*[円¥]/m);
      if (yenMatch) {
...
function matchPdfUsingINumericPrice2() {
  ...
  function getFileFromPrice(pr){
    ...
    if( fs.length==0){
      invalidFiles.forEach((fn)=>{
        let pat=getDisp(pr);
        let pattern = `[^,\\d]${pat}\\s*(万)`, text;
        text=dic[fn].text?dic[fn].text:'';
        let regex = new RegExp(pattern,'m'), match=text.match(pattern);
        if (match) fs.push(fn); else{
          pat=pat.replace(',',''); pattern = `[^,\\d]${pat}\\s*(万)`; regex = new RegExp(pattern,'m'); match=text.match(pattern);
          if(match){ fs.push(fn); }else{
            pat=getDisp(pr, 1); pattern = `[^億,\\d]${pat}\\s*(万)`; regex = new RegExp(pattern,'m'); 
            match=text.match(pattern);
            if(match){ fs.push(fn);}else{
              pr*=10000; pat=getDisp(pr); pattern = `[^,\\d]${pat}\\s*(円)`; regex = new RegExp(pattern,'m'); 
              match=text.match(pattern);
              if(match){ fs.push(fn);}
            }
          }
  ...
  function getFileFromAddr(addr){
    const fs=[]; addr=toHalfWidth(addr); addr= addr.replace(/丁目/,'');
    let addr2=addr.replace(/^(北海道|青森県|岩手県|宮城県|秋田県|山形県|福島県|茨城県|栃木県|群馬県|埼玉県|千葉県|東京都|神奈川県|新潟県|富山県|石川県|福井県|山梨県|長野県|岐阜県|静岡県|愛知県|三重県|滋賀県|京都府|大阪府|兵庫県|奈良県|和歌山県|鳥取県|島根県|岡山県|広島県|山口県|徳島県|香川県|愛媛県|高知県|福岡県|佐賀県|長崎県|熊本県|大分県|宮崎県|鹿児島県|沖縄県)/,''), addr3=addr2;
    ...
    for(let di in dic){
      ...
      if(di.indexOf('レインズ資料 4180万円.pdf')==0)
        b++;
      let pattern = `${addr}`;
      let regex = new RegExp(pattern), match=dic[di].text.match(pattern);
      if (match) fs.push(di);
      else{        
        pattern = `${addr2}`; regex = new RegExp(pattern), match=dic[di].text.match(pattern);
        if (match) fs.push(di);
        else{   
          pattern = `${addr3}`; regex = new RegExp(pattern), match=dic[di].text.match(pattern);
          if (match) fs.push(di);
        }
      }
    }
  ...
  const data = sheet.getDataRange().getValues();
  const dic={}, invalidFiles=[];
  const allFiles=DriveApp.getFilesByName('allText.txt');
  const allFile = allFiles.next();
  let allText=allFile.getBlob().getDataAsString(); allText=convertNFC(allText);
  ...
  data.forEach((dat, index)=>{
    let price=parseInt(toHalfWidth(dat[8]).trim().replace(/,/g,'')), fn='', addr=dat[5].trim();
    ...
    if(fs.length==1){
      ...
      if(fa.length==1) { 
        ...
        fn='MULTI_PRICE_ADDR'; isPriceMatch=1;    
      }
      else fn=fa[0];
    }
    if(isPriceMatch == 0){
    ...
    if( fn.endsWith('.pdf')){
      let rn=fn; if(rn.indexOf('ADDR_MATCH')===0) rn=rn.substring(10);
      let url=getFileUrlByName('1jwQ03EQ0O7Ofl8geDH21v1HXom8NNiH7',rn);
