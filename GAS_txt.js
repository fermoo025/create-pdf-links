let dic = {},
    invalidFiles = [];
const FOLDER_ID = '1cNXR9xAzRoi5PsiAQwknTvkQsx7a53S2',
...
function trashFilesByName(filename) {
...
function matchPdfUsingINumericPrice() {
    ...
    dic = {};invalidFiles = [];
    let allText=''; trashFilesByName('allText.txt');
    while (files.hasNext()) {
    ...
    DriveApp.createFile('allText.txt', allText);
    common();
}
...
function common() {
    ...
    data.forEach((dat, index) => {
        let price = parseInt(toHalfWidth(dat[8]).trim().replace(/,/g, '').replace('万円','')),
            fn = '',
            addr = dat[6].trim();
