...
function convertPdfs( subFolders ) {
  const tic = Date.now();
  subFolders.forEach(folder=>{
    const dic = {}, files = folder.getFiles();
    while (files.hasNext()) {
    ...
    for (let fn in dic) {
      if (dic[fn] == 0) {
        ...
        if (Date.now() - tic > 270000) { // updateStatus('yet'); 
          return 'yet';
        }// timeout;
      }
    }
  });
  console.log('convert done');  return 'done'; //updateStatus('done');   
}
function downTexts(subFolders, pName) {
  let allText = '';
  subFolders.forEach(folder=>{
    const fName = folder.getName(), files = folder.getFiles();
    while (files.hasNext()) {
...
function doPost(e) {
  let data = JSON.parse(e.postData.contents), subs = data.subFolders.trim().split(/\s+/);  
  let subs2 = [ ...subs];
  const sheetId = data.sheetId, sheetName = data.tabName, folderId = data.folderId, subFolders = [];
  let ret;
  const pFolder = DriveApp.getFolderById(folderId); if( pFolder == null ) return ContentService.createTextOutput('bad folderId');
  const pName = pFolder.getName();
  let folders = pFolder.getFolders();
  while (folders.hasNext()){ 
    const folder = folders.next(), fName = folder.getName(), pos=subs2.indexOf(fName);
    if( pos > -1 ){ subFolders.push(folder); subs2.splice(pos, 1); }
  }
  if( subs2.length > 0 ){ return ContentService.createTextOutput('bad subFolders:'+ subs2.join(' ')); }
  if (data.command == 'convert') {
    ret = convertPdfs( subFolders );
    if (ret == 'yet') {

    } else if (ret == 'done') {
      const allText = downTexts(subFolders, pName );
      var spreadsheet = SpreadsheetApp.openById(sheetId);
      if( spreadsheet == null ) ret = 'bad sheetId';
      else{
        var sheet = spreadsheet.getSheetByName(sheetName);
        if (sheet == null ) ret = 'bad tabName';
        else{ 
          ...
          lines.forEach((line) => {
            ...
            const match = line.match(/(.+\/.+\/.+\.pdf)===(https:\/\/.+)/, line);
            if (match) {
              path = match[1]; dic[path] = { 'url': match[2], 'text': '' };
            } else { dic[path]['text'] += line + '\n'; }
          });
          ret = common(data, sheet, folderId);
        }
      }
    }else ret = 'convert error';
  } else ret = 'bad command';
  return ContentService.createTextOutput(ret);
}

function common(data, sheet, folderId) {
  for (const fn in dic) {
    const text = dic[fn].text;
    const manMatch = text.match(/価\s*格(?:[:\s]|\(\s*税\s*込\s*\))*([\d,億]+)\s*万/m);
    let priceValue = 0;
    if (manMatch) {
      priceValue = parseInt(manMatch[1].replace(/[,億]/g, ''), 10) * 1000;
    } else {
    ...
    if (priceValue == 0) invalidFiles.push(fn);
  }
  //DriveApp.createFile('allPdfTxt', allText, MimeType.PLAIN_TEXT);
  data.shift();
  data.forEach((dat, index) => {
    let fn = '', addr = dat[6].trim();
    if (dat[2].trim()!='' || addr =='') return;
    let txtPrice = toHalfWidth(dat[8]).trim().replace(",", ""), multi = 1;
    if ( txtPrice.endsWith("万円") ){ txtPrice = txtPrice.replace("万円", ""); multi = 10000;    }
    else if ( txtPrice.endsWith("円") ){ txtPrice = txtPrice.replace("円", ""); }
    let price = parseInt(txtPrice) * multi;
    let fs = getFileFromPrice(price);
    let isPriceMatch = 0, fa = [];
    if (fs.length == 1) {      //let a = 1;      if (price == 4180)        a++;
      fa = getFileFromAddr(addr);
      if (fa.length == 1) {
        fn = fa[0]; isPriceMatch = 1; delete dic[fn];
        let pos=invalidFiles.indexOf(fn);
        if(pos!=-1) invalidFiles.splice(pos, 1);
      } else if (fa.length > 1) {
        fn = 'MULTI_PRICE_ADDR';
        isPriceMatch = 1;
      } //else fn = fa[0];
    ...
    if (isPriceMatch == 0) {
      if (fs.length > 1) {
        ...
        if (fs.length == 1) {
          fn = 'ADDR_MATCH' + fs[0];  delete dic[fs[0]];
          let pos=invalidFiles.indexOf(fs[0]);
          if(pos!=-1) invalidFiles.splice(pos, 1);
        } else if (fs.length > 1)
          fn = 'MULTI_ADDR';
        else
    ...
    if (fn.endsWith('.pdf')) {
      ...
      let pos = rn.indexOf('/'), fName=rn.substring(0,pos), pdfName= rn.substring(pos+1);
      const folder = DriveApp.getFolderById(folderId);
      const folders = folder.getFoldersByName(fName);
      let sFolder = folders.next();
      const files = sFolder.getFilesByName(pdfName);
      const file = files.next();
      let url = file.getUrl();
